...
| [11](AdventOfCode2025/Day11.cs) | 🟪 | After the nightmares the last three days had been, today's was a relief. The solution is four parts: 1. parse the input, i already had made a utility function for this. 2. recursive DFS-Dijkstra's-looking thing that counts all paths from any node to any other. 3. memoisation. 4. shortest path from A -> B that goes through C equals shortest path from A -> C, followed by shortest path from C -> B, this fact applies the same say to counting paths. |
| [12](AdventOfCode2025/Day12.cs) | 🟣 | What a WILD final day... Of course, I first wrote a recursive solution, then after realising it was intractable I thought perhaps another LP formulation, or DLX. I decided before I dove into those I'd quickly check which regions *were actually feasible*, ie. the sum of all the areas of the presents is less than the total area of the region. Surprise, surprise, over half of the setups weren't even feasible. Furthermore, I noticed those that were feasible were actually FAR below the full area, all at least 364 square units of emptiness... Seems feasible enough? Lo and behold, every 'feasible' region could fit all the presents. And for a more mathematically rigorous method, the presents can definitely fit if there are enough 3x3s to just put a present in each one (no need for interlocking). So our solution is: 1. if not feasible, reject. 2. if definitely arrangeable, accept, 3. otherwise, brute force. And our input happens to be such that every setup is either infeasible or definitely arrangeable, so we never have to brute force! (My brute-force solution is still visible in the git history, but I've removed it from my current solution. |